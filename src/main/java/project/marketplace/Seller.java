...
public abstract class Seller extends User {
    ...
    public static int load() {
        ...
        while(rowIt.hasNext()) {
            ...
            cell = row.getCell(1);
            boolean admin = cell.getStringCellValue().equals("ADMIN");

            cell = row.getCell(2);
            String name = (Objects.isNull(cell)) ? "" : cell.getStringCellValue();

            Seller seller;
            if (admin) { seller = new Admin(id); } else { seller = new ExternSeller(id, name); }
    ...
    protected Seller(int id, String name) {
        this.id = id;
        this.name = name;
    }
